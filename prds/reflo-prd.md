...
**Version:** 1.4 · **Date:** July 17, 2026 · **Status:** Approved for build sprint
**Changelog:** v1.4 — delivery/scheduling, scope membership, exam mapping, per-concept evidence, tutor-turn provenance, and deletion-store models added; performance and re-teach gates made executable; TTS fallback, prompt-injection tests, pilot consent, D7 event semantics, email interaction, and offline-demo envelope specified; OAuth moved to P1
...
- For an item tagged to multiple concepts, grading produces an immutable per-concept evidence outcome (score, rubric band, confidence, and rationale reference) rather than copying one overall grade to every concept. Partially correct answers update only concepts with confident evidence. Every outcome stores grader model, prompt, rubric, threshold, and algorithm versions so the update can be reproduced.
- Anti-pattern guard: never repeat the identical question within a session; vary surface form to test the concept, not memorization of the item. Every attempt references its session so this rule is enforceable.
...
- Persist each due review separately from each delivery attempt. A review schedule records learner/concept, due time/time zone, FSRS version, state, and reschedule reason. One delivery batches 1–3 due reviews for a channel and records channel identity, provider message ID, idempotency key, status/attempt count, expiry, error, and timestamps; delivery items link each included review schedule to its quiz item and order. Provider message IDs and idempotency keys are unique per provider; retries reuse the logical delivery and cannot create duplicate attempts.
- Function Compute cron triggers; fallback to email only when the learner has opted into email delivery. Inbound messaging webhooks require signature verification, replay protection, a provider submission ID persisted as the attempt idempotency key, and an explicit account-linking flow before an answer can update learner state. A uniqueness constraint prevents the same provider submission from creating two attempts. Ambient answers reference `delivery_item_id`; `session_id` is nullable and is set only when an app session exists.
...
- Deletion removes or irreversibly crypto-shreds learner-linked PII/content from RDS (including the otherwise append-only event ledger), vector namespaces, OSS/CDN, Redis, RocketMQ and dead-letter queues, channel identities/deliveries, SLS, Langfuse, evaluation exports, and identifiable pilot-analysis datasets. The deletion job has per-store checkpoints, retries, terminal failure visibility, and an audit record containing no deleted identifiers. Active/derived stores complete within 24 hours; encrypted backups expire within 30 days. Only previously produced, irreversibly anonymized aggregates may remain, subject to the learner's recorded consent. The UI states these rules before confirmation.
...
`User` (id, auth, prefs, channel opt-ins) · `ChannelIdentity` (user, provider, encrypted_external_id, verified_at, revoked_at) · `OwnerScope` (id, type: user|organization; MVP creates user scopes only) · `ScopeMembership` (owner_scope_id, user_id, role, active_from, revoked_at; unique active user-scope owner) · `Course` (id, owner_scope_id, source_document_id, target_exam_blueprint_id?, status) · `SourceDocument` (id, owner_scope_id, OSS URI, checksum, parse_status, retention_status) · `SourceSpan` (id, source_document_id, chunk_text, page/section/start/end, embedding_ref) · `ExamBlueprint` (id, name, version, evidence_minimums) · `ExamObjective` (id, blueprint_id, code, weight, mapping/evidence_minimums, version) · `ConceptObjectiveMapping` (concept_id, objective_id, weight, source/provenance, mapping_version, review_status, invalidated_at?) · `Chapter` (course_id, order, title, source_span_ids[]) · `Concept` (chapter_id, name, prerequisite_ids, source_span_ids[], generation_version) · `Asset` (id, chapter_id?, concept_id?, type: audio|video|text, OSS URI, source_span_ids[], model/prompt/version, strategy_tag, status) · `QuizItem` (id, course_id, concept_ids[], source_span_ids[], type, difficulty, keyed_answer, rubric, version) · `ReviewSchedule` (id, user, concept, due_at, time_zone, fsrs_version, state, reschedule_reason) · `QuizDelivery` (id, channel_identity_id, provider_message_id, idempotency_key, status, attempt_count, expires_at, error, timestamps; provider/message and provider/idempotency unique) · `DeliveryItem` (id, delivery_id, review_schedule_id, quiz_item_id, order; unique delivery/schedule) · `Session` (id, user, plan, actions[], summary) · `Attempt` (id, user, session_id?, delivery_item_id?, submission_idempotency_key?, quiz_item_id, answer, outcome: graded|abstained|superseded, overall_grade?, grading_confidence, grader_model/prompt/rubric/threshold versions, ts; provider/submission unique) · `AttemptConceptEvidence` (attempt_id, concept_id, score, rubric_band, confidence, rationale_ref, eligible_for_mastery) · `TutorTurn` (id, user, session_id, request, response, source_span_ids[], model/prompt/version, grounding_status, ts) · `LearningEvent` (id, user, session_id?, delivery_id?, concept_ids[], type, idempotency_key?, payload, ts) · `KnowledgeState` (user, concept, mastery, confidence, half_life, last_review, review_count, algorithm_version) · `ConsentRecord` (user, consent_version, purposes, experiment_version?, granted_at, withdrawn_at?) · `ExperimentAssignment` (user, concept, consent_version, experiment_version, arm, strata, assigned_at, withdrawn_at?) · `DeletionJob` (user, requested_at, per_store_status/retries, backup_expiry, terminal_status) · `AgentTrace` (pseudonymous Langfuse ref)
...
**Week 2 (Jul 24–30) — The loop.** Knowledge model + FSRS scheduling, adaptive quiz selection, executable Tutor Agent re-teach behavior, Knowledge Map UI, durable Telegram/email scheduling and delivery, Langfuse/SLS wiring, and the seeded offline bundle. WhatsApp remains P1. *Exit: the precise Flow B assertion passes online and offline; delivery retries/webhook replays create no duplicate attempts; **first 5 consented pilots live by Jul 30** (not merely invited — D7 retention needs a week of runway before the Aug 7 metrics cut).*

**Week 3 (Jul 31–Aug 7) — Pilots & polish.** 10–20 pilot learners live, grounding/grading/quiz eval suites run and release gates met, ROCm benchmark with AMD mentors, eligible readiness-score calibration, and demo hardening (offline fallbacks, seeded demo course). Voice mode and full-course video remain P1 and are built only if all Week 1–2 P0 exit criteria passed on time. *Exit: seeded Flow B runs reliably in ≤ 6 min; the upload SLO is demonstrated separately; provisional pilot metrics report eligible cohorts and denominators.*
