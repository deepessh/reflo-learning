...
**Version:** 2.6 · **Date:** July 27, 2026 · **Status:** Approved for build sprint
**Changelog:** v2.6 — narrowed Demo Day ingestion to digitally generated PDF; retained the 20 MB/200-page standard profile and 50 MB/800-page maximum for PDF; moved EPUB and DOCX ingestion, format-native limits, and their release-gate evidence to post–Demo Day fast-follow
...
- **G1:** For a supported, digitally generated PDF within the standard-ingestion profile (≤ 20 MB, ≤ 200 pages, no OCR), a learner receives a browsable curriculum outline at p95 ≤ 16 minutes, the first text micro-lesson plus placement quiz at p95 ≤ 5 minutes, and chapter 1 audio at p95 ≤ 10 minutes. PDFs up to the 50 MB/800-page product maximum remain supported but use an asynchronous large-document path with an estimate/status, as do OCR jobs; neither carries the 16-minute SLO. EPUB and DOCX ingestion are post–Demo Day fast-follow and carry no sprint support or SLO claim. Remaining audio and quizzes generate progressively. Video is a P1 enhancement and never part of an activation SLO.
...
- Standard-ingestion profile for activation SLOs: digitally generated PDFs ≤ 20 MB and ≤ 200 pages that do not require OCR. Larger supported PDFs use the asynchronous large-document path. EPUB and DOCX are unavailable in the Demo Day product surface, are rejected before processing with PDF-only guidance, and move to §7.
...
- Output for the standard-ingestion profile: a course object with per-chapter concept lists visible at p95 ≤ 16 minutes; media generates progressively in the background. SLO measurement begins after upload completion and ends when the outline is usable.
...
**Flow A — First course (demo activation):** Enter a labeled demo identity → select or operator-upload a rights-cleared standard-profile PDF → watch curriculum appear (p95 ≤ 16 min) → take the 10-question placement quiz → see the initial Knowledge Map → complete the 2–3 minute chapter 1 text micro-lesson → chapter quiz → map updates. *Target: upload complete → first text micro-lesson completed at p95 ≤ 15 minutes; OCR documents are excluded and show a separate estimate. EPUB and DOCX are not accepted by the Demo Day product surface.*
...
- Live adaptive loop on a pre-ingested seeded course: failed question → materially different micro-lesson → re-test → evidence-based map update (≤ 6 min). Demonstrate standard-profile PDF upload → outline separately against the p95 ≤ 16-minute SLO; do not imply that full ingestion and media generation complete inside the 6-minute loop.
