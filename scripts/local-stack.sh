...
worker_status() {
  if ! command -v node >/dev/null 2>&1; then
    echo "UNSUPPORTED local-workers: install the repository-pinned Node.js before checking the optional profile."
    echo "SKIPPED local-workers: optional ingestion and Piper readiness was not evaluated."
  else
    if ! node "$REFLO_LOCAL_ROOT/scripts/local-workers.mjs" status; then
      echo "INVALID local-workers: readiness command failed; run corepack pnpm local-workers:prepare."
    fi
  fi
}
