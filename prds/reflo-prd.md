...
**Version:** 2.5 · **Date:** July 27, 2026 · **Status:** Approved for build sprint
**Changelog:** v2.5 — doubled the standard-profile upload-to-outline SLO again from eight to sixteen minutes after connected Qwen evidence partially completed the partition set but still exhausted the 120-second child ceiling; retained the standard profile, grounding, demo-only boundary, and all later activation SLOs
...
- **G1:** For a supported, digitally generated source within the standard-ingestion profile (PDF/EPUB/DOCX, ≤ 20 MB, ≤ 200 pages, no OCR), a learner receives a browsable curriculum outline at p95 ≤ 16 minutes, the first text micro-lesson plus placement quiz at p95 ≤ 5 minutes, and chapter 1 audio at p95 ≤ 10 minutes. Files up to the 50 MB/800-page product maximum remain supported but use an asynchronous large-document path with an estimate/status, as do OCR jobs; neither carries the 16-minute SLO. Remaining audio and quizzes generate progressively. Video is a P1 enhancement and never part of an activation SLO.
...
**Flow A — First course (demo activation):** Enter a labeled demo identity → select or operator-upload a rights-cleared standard-profile study guide → watch curriculum appear (p95 ≤ 16 min) → take the 10-question placement quiz → see the initial Knowledge Map → complete the 2–3 minute chapter 1 text micro-lesson → chapter quiz → map updates. *Target: upload complete → first text micro-lesson completed at p95 ≤ 15 minutes; OCR documents are excluded and show a separate estimate.*
...
- Live adaptive loop on a pre-ingested seeded course: failed question → materially different micro-lesson → re-test → evidence-based map update (≤ 6 min). Demonstrate standard-profile upload → outline separately against the p95 ≤ 16-minute SLO; do not imply that full ingestion and media generation complete inside the 6-minute loop.
