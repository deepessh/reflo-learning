...
**Version:** 2.7 · **Date:** July 27, 2026 · **Status:** Approved for build sprint
**Changelog:** v2.7 — prioritized deployment and hardening of the complete staff-controlled connected application; deferred formal target-production performance, dual-TTS, upload-security, adversarial-document, and secure-ingestion environment qualification until after Demo Day; retained the v2 evaluation contracts, historical evidence, fixtures, secure behavior, and honest no-qualification labeling as future verification capability
v2.6 — narrowed Demo Day ingestion to digitally generated PDF; retained the 20 MB/200-page standard profile and 50 MB/800-page maximum for PDF; moved EPUB and DOCX ingestion, format-native limits, and their release-gate evidence to post–Demo Day fast-follow
v2.5 — doubled the standard-profile upload-to-outline SLO again from eight to sixteen minutes after connected Qwen evidence partially completed the partition set but still exhausted the 120-second child ceiling; retained the standard profile, grounding, demo-only boundary, and all later activation SLOs
v2.4 — doubled the standard-profile upload-to-outline SLO again from four to eight minutes after all first-wave connected Qwen children exhausted the 60-second ceiling; retained the standard profile, grounding, demo-only boundary, and all later activation SLOs
...
- **G1:** Retain these post–Demo Day release-qualification targets for a supported, digitally generated PDF within the standard-ingestion profile (≤ 20 MB, ≤ 200 pages, no OCR): a browsable curriculum outline at p95 ≤ 16 minutes, the first text micro-lesson plus placement quiz at p95 ≤ 5 minutes, and chapter 1 audio at p95 ≤ 10 minutes. Formal target-production qualification against those p95 targets is not a sprint or Demo Day gate. The staff-controlled Demo Day upload records its observed latency and outcome without claiming p95, capacity, or production qualification. PDFs up to the 50 MB/800-page product maximum remain supported but use an asynchronous large-document path with an estimate/status, as do OCR jobs; neither carries the 16-minute target. EPUB and DOCX ingestion are post–Demo Day fast-follow and carry no sprint support or SLO claim. Remaining audio and quizzes generate progressively. Video is a P1 enhancement and never part of an activation SLO.
...
- **G5:** Deploy, harden, and deliver a reliable, honestly labeled connected demo through August 15 using only seeded, synthetic, or staff-controlled test identities and human-approved rights-cleared sources. No external learner is recruited or activated before Demo Day; the real-user pilot begins post-hackathon.
...
- EPUB and DOCX upload support and their format-native eligibility and evaluation gates (fast-follow; see §7)
...
- Upload digitally generated PDF only for Demo Day (≤ 50 MB and ≤ 800 pages)
- Standard-ingestion profile for the retained post–Demo Day qualification targets: digitally generated PDFs ≤ 20 MB and ≤ 200 pages that do not require OCR. Larger supported PDFs use the asynchronous large-document path. EPUB and DOCX are unavailable in the Demo Day product surface, are rejected before processing with PDF-only guidance, and move to §7.
...
- Output for the standard-ingestion profile: a course object with per-chapter concept lists; media generates progressively in the background. The retained post–Demo Day qualification target is p95 ≤ 16 minutes from upload completion until the outline is usable. Demo Day records the observed staff-controlled run without presenting it as a p95 result.
...
- Keep the implemented primary TTS path and quota-independent fallback behind the same learner-visible asset behavior, with explicit unavailable/degraded states. Formal target-environment capacity and dual-path quality/SLO qualification move after Demo Day and must not be claimed from development smoke results. Any paid capacity still requires the human spending approval in `AGENTS.md`.
...
- Conversational tutoring: the learner can ask questions anytime outside an active assessment. During an active assessment, Tutor unlocks only after a successfully graded, non-abstained attempt; abstained or failed-to-grade submissions do not unlock it. Answers are RAG-grounded in the source with citations to chapter/section.
...
- Fallback to email only when the learner has opted into email delivery. Inbound messaging requires authenticated provider updates, replay protection, and explicit account linking before an answer can update learner state. Deployed environments use authenticated HTTPS callbacks. The local staff-only Demo Day runtime may instead use mutually exclusive Telegram long polling over the authenticated Bot API; it must feed the same validation, authorization, replay, and evidence path. Provider retries, webhook replays, or repeated poll results cannot create duplicate attempts, and ambient answers remain linked to the delivered item.
...
- **EPUB and DOCX ingestion:** expose the existing parser groundwork only after defining deterministic format-native product limits, building representative rights-cleared evaluation corpora, validating security and latency, and updating the product support contract; do not use renderer-derived or invented page counts
...
- **Formal release qualification:** execute the retained v2 target-production performance, dual-TTS, upload-security, adversarial-document, and secure-ingestion environment suites; publish current attestations; and resolve any failures before making production-readiness or post-hackathon pilot claims

---

## 8. User Flows (primary)

**Flow A — First course (demo activation):** Enter a labeled demo identity → select or operator-upload a rights-cleared standard-profile PDF → watch the curriculum appear and record the observed elapsed time → take the 10-question placement quiz → see the initial Knowledge Map → complete the 2–3 minute chapter 1 text micro-lesson → chapter quiz → map updates. *The retained post–Demo Day qualification targets are outline p95 ≤ 16 minutes and upload complete → first text micro-lesson completed at p95 ≤ 15 minutes. The Demo Day run is an observation, not p95 or production qualification. OCR documents are excluded and show a separate estimate. EPUB and DOCX are not accepted by the Demo Day product surface.*
...
- **Deferred performance qualification:** retain the v2 benchmark contract and future corpus profile of at least 40 rights-cleared, standard-profile PDFs spanning 5–200 pages, 0.5–20 MB, tables/images, and simple/complex chapter structures. Post–Demo Day qualification runs from cold application/model caches in the target deployment with five concurrent ingestions and at least three runs per document. It measures upload-complete → usable outline, first rendered text micro-lesson + complete 10-question placement quiz, and playable chapter 1 audio against the G1 p95 targets; retries, terminal failures, and timeouts count as misses. A "usable outline" means the course opens, every detected chapter has a title and at least one source-backed concept or an explicit empty/error state, and source links resolve; the 5-minute activation package requires rendered source-backed text plus ten answerable/source-backed placement items. Publish benchmark version, environment, sample count, misses, and latency distribution. EPUB and DOCX remain ineligible. This suite is not a sprint or Demo Day blocker, and a staff-controlled demonstration or small favorable sample cannot be presented as its pass.
- **Deferred audio qualification:** retain the v2 profile of at least 30 representative chapter scripts through both primary and fallback TTS paths under the same five-course concurrency profile. Post–Demo Day, each path must meet chapter 1 p95 ≤ 10 minutes, produce a playable authorized asset, and have zero unintelligible samples in two-reviewer listening QA at 1.0× and 1.5×. Record capacity/quota evidence then; development playback and Demo Day use do not qualify either path or establish production capacity.
- **Secure-ingestion behavior and deferred environment qualification:** PDF type, malware, encryption, size/page, isolation, network/credential denial, owner-scope, cleanup, and explicit EPUB/DOCX early-rejection behavior remain required. Keep the v2 upload-security contract, ingestion fixtures, historical evidence, and accepted secure-ingestion architecture intact. Formal vulnerability, immutable-registry/OSS, scanner-snapshot admission, Podman/ECS, and target-environment upload-security qualification moves after Demo Day. Deployment smoke checks are useful defect evidence but cannot be called a gate pass.
...
- **Deferred adversarial-document qualification:** retain the v2 suite of at least 20 supported PDFs containing indirect prompt injection, fake citations, cross-scope references, grading-manipulation instructions, and tool-use requests. Post–Demo Day qualification requires zero cross-scope disclosures, authorization/tool-policy changes caused by source text, execution of source-supplied instructions, or displayed citations that do not resolve server-side to an authorized source span. Any failure found before then is still disclosed and fixed or honestly bounded; absence of the formal run is never represented as proof that the behavior is qualified or production-ready.
- **Calibration:** An Exam Readiness Score is eligible for display only for a versioned exam blueprint and only after its configured evidence minimum is met. Validate it against practice-exam scores where available and show the sample size and error; until validation is adequate, label it "experimental." All other courses show assessment coverage in the top-level summary and concept-level mastery estimates, without a course-wide mastery aggregate.
...
- Live adaptive loop on a pre-ingested seeded course: failed question → materially different micro-lesson → re-test → evidence-based map update (≤ 6 min). Demonstrate one staff-controlled standard-profile PDF upload → outline separately and report its exact observed elapsed time; do not describe that run as p95 or production qualification, and do not imply that full ingestion and media generation complete inside the 6-minute loop.
...
- Architecture story told from accepted targets and evidence-backed implemented state; video is included only if the P1 flag shipped, otherwise shown as a labeled prototype/benchmark; any accelerator or release-qualification result is reported honestly, including when it was not run
...
**Week 1 (Jul 17–23) — Pipeline & skeleton.** Secure PDF ingestion end-to-end with source spans, primary + fallback audio generation, authorized private-asset delivery, demo identity + library UI, quiz generation v1, and one nominal 15-second source-backed video prototype behind the P1 video flag. Freeze the rights-cleared seeded demo source and synthetic/test identities. *Exit: the core pipeline works with the seeded course. Repository evaluation contracts and local evidence are retained, but formal target-environment release qualification is not a Week 1 or Demo Day exit gate and is never implied to have passed.*
...
**Week 3 (Jul 31–Aug 7) — Deployment & demo polish.** Prioritize deployment and hardening of the complete staff-controlled connected application on Alibaba Cloud, plus non-blocking quality regression checks. Run the planned accelerator benchmark or other formal qualification only if it does not displace the connected app; otherwise defer it and say so. Voice mode remains P1 and is built only if all Week 1–2 P0 exit criteria passed on time; production 60–120 second composition and full-course video generation remain post–Demo Day fast-follow work. *Exit: seeded Flow B runs reliably in ≤ 6 min, the separate PDF upload succeeds with exact observed timing, dependency failures remain honest, and no p95, release-gate, security-qualification, production-readiness, or pilot claim exceeds the evidence.*
...
| Audio quota/capacity delays block P0 audio | Medium | Keep primary and quota-independent fallback implementations, preflight the path used by the staff-controlled demo, and show an honest unavailable/degraded state. Formal dual-path capacity and quality qualification is post–Demo Day and is not claimed from demo playback. |
...
| Untrusted document or unauthorized asset access | Medium | Validate and scan uploads, isolate parsers/OCR, enforce expansion/resource limits, use owner-scoped retrieval and short-lived signed URLs, and smoke-check those controls in the deployed staff-only flow. Formal secure-ingestion and upload-security environment qualification remains required after Demo Day before any production-readiness or external-use claim. |
| Deferred qualification is mistaken for a pass | High | Keep the v2 harness, schemas, fixtures, historical evidence, and accepted architecture intact; label formal gates as not run or unqualified; and distinguish deployment and smoke evidence from target-environment attestations in every demo and artifact. |
...
| Too many P0s for a 3-person team | High | Preserve P0 PDF ingestion, audio+text, assessment, knowledge model, re-teach loop, test-only Telegram/email delivery, and the demo identity boundary. EPUB/DOCX are the owner-approved ingestion deferral. Keep video bounded to the single default-off prototype; defer production video, P1 voice, and real-user accounts first. Any further cut requires the human escalation process in `AGENTS.md`. |
