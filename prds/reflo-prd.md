...
*Authority is split by role. This PRD controls product requirements, scope, architecture mandates, priorities, and release gates. GitHub `decision` issues hold proposals, evidence, discussion, and authorization; a verdict becomes effective only when its matching record merges into `DECISIONS.md`. This section plus effective decision records are the implementation source of truth for infrastructure. The PRD wins over any contradictory issue, register entry, external form, or code. A register entry without matching authorization is invalid, and a GitHub verdict without a merged register entry is not yet effective. PRD-mandated choices can change only through a PRD revision. The one-time issue-free bootstrap exception for introducing this governance system is recorded in `AGENTS.md` and `DECISIONS.md`.*
...
- **Vector store — PRD mandate:** AnalyticDB for PostgreSQL (pgvector-compatible) for the sprint — one fewer moving part and Postgres-native ops for a 3-person team; migrate to Milvus only if corpus scale demands it post-launch. Builder Day feedback is advisory; record it in the relevant `decision` issue and PRD mandate index, and do not change the selected store without revising this PRD.
- **Security:** isolated VPC, encryption at rest, Stripe tokenization (no card data), owner-scope-isolated content embeddings, least-privilege worker identities, short-lived signed asset URLs, isolated upload parsing, authenticated channel webhooks, PII-minimized model prompts, and auditable deletion jobs. Messaging/payment providers may process only the minimum delivery or transaction metadata required and only after explicit user opt-in and disclosure; vendor retention, training, and deletion settings must be verified before pilots. Every course and source document carries an `owner_scope_id`; descendant records and vector namespaces inherit that scope. A user scope must have exactly one active owner membership. Organization scopes and non-owner roles are schema-ready but cannot be created in MVP. API authorization and database constraints must verify active membership before retrieval, asset signing, mutation, or vector search.
